...
def verify_str_arg(value, arg=None, valid_values=None, custom_msg=None):
    if not isinstance(value, torch._six.string_classes):
        ...
        if arg is None:
            msg = f"Expected type str, but got type {type(value)}."
        else:
            msg = f"Expected type str for argument {arg}, but got type {type(value)}."
    ...
    if value not in valid_values:
        if custom_msg is not None:
            ...
            msg =  (f"Unknown value '{value}' for argument {arg}. "
                    f"Valid values are {{{iterable_to_str(valid_values)}}}.")
