    ...
    if not feature_list:
        ...
        for i in range(len(mat_list)):
            ...
            try:
                ...
                print(f'SVD did not converge for layer {i+1}, using alternative method')
                U1,S1,Vh1=np.linalg.svd(activation + 1e-10 * np.random.randn(*activation.shape), full_matrices=False)
            ...
            try:
                ...
                print(f'SVD did not converge for act_hat in layer {i+1}, using alternative method')
                U,S,Vh = np.linalg.svd(act_hat + 1e-10 * np.random.randn(*act_hat.shape), full_matrices=False)
...
def main(args):
    ...
    for k,ncla in taskcla:
        ...
        threshold = np.array([0.965] * 20) + task_id * np.array([0.003] * 20)
...
if __name__ == "__main__":
    # Training parameters
    parser = argparse.ArgumentParser(description='20-split miniImageNet with GPM')
    ...
    for seed_ in [3, 37]:
        ...
        try:
            ...
            print(f"seed {seed_} Error: {type(e).__name__}: {str(e)}")
            import traceback
