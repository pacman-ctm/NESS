...
def main(args):
    ...
    for k,ncla in taskcla:
        ...
        if task_id==0:
            ...
            for epoch in range(1, args.n_epochs+1):
                ...
                acc_list.append(valid_acc)
                # flag = checking_inner(args, model, memory, task_name, task_id, task_name_list, epoch) # Debug
    ...
    # Debug: Pad all lists to the same length with NaN
    max_len = max(len(lst) for lst in acc_list_all)
    acc_list_padded = []
    for lst in acc_list_all:
        padded = lst + [np.nan] * (max_len - len(lst))
        acc_list_padded.append(padded)
    np.save('five_ours.npy', np.array(acc_list_padded))

    print_log('-'*50, log)
    # Simulation Results 
    print_log ('Task Order : {}'.format(np.array(task_list)), log)
    print_log ('Final Avg Accuracy for seed {}: {:5.2f}%'.format(args.seed, acc_matrix[-1].mean()), log) 
